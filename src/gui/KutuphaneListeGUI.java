...
public class KutuphaneListeGUI extends JFrame {
    ...
    private void talepEtAction() {
        ...
        if (ogrenciNo != null && !ogrenciNo.trim().isEmpty()) {
            ...
            if (ogrenciler.containsKey(ogrenciNo)) {
                ...
                try {
                    // Sadece Talep Oluştur (Durum -> Bekliyor)
    ...
    private void iadeEtAction() {
        // İade kodu aynı kalabilir, yukarıdakiyle aynı mantıkta.
        // Sadece güvenlik için numara sorar ve iade alır.
        int row = table.getSelectedRow();
        if (row == -1) { JOptionPane.showMessageDialog(this, "Kitap seçiniz."); return; }
        String isbn = (String) model.getValueAt(table.convertRowIndexToModel(row), 2);

        try {
            DosyaIslemleri.kitapIadeEt(isbn);
            JOptionPane.showMessageDialog(this, "İade alındı.");
            verileriYukle();
